...
use App\Http\Requests\Master\Warehouse\StoreUpdateRequest;
use App\Models\User;
...
class WarehouseController extends Controller
{
    ...
    {
        return view('master.warehouse.list');
    }
    ...
    public function getData(Request $request)
    {
        ...
        if(is_null($request->search['value'])) {
            ...
            $total_data = Warehouse::where('name', 'like', '%'.$search.'%')
        ...
        if(count($query->get()) > 0) {
            ...
            foreach($query->get() as $value) {
                $arr_data = array(
                    ...
                    'action' => '<a href="'.route('master.warehouse.detail', ['id' => $value->id]) .'"
    ...
    public function store(StoreUpdateRequest $request)
    {
        try {
            DB::beginTransaction();
            $code = GlobalHelpers::generateCode('code_warehouse',date('Y'),date('m'));
            $input = array(
                'code' => $code,
    ...
    public function detail($id)
    {
        ...
        return view('master.warehouse.detail', compact('data'));
    }
