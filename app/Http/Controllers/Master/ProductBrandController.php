...
use App\Http\Requests\Master\ProductBrand\StoreUpdateRequest;
...
class ProductBrandController extends Controller
{
    ...
    {
        return view('master.product_brand.list');
    }

    public function getData(Request $request)
    {
        $total_data = ProductBrand::count();

        $limit = $request->length;
        $start = $request->start;
        
        if(is_null($request->search['value'])) {
            $query = ProductBrand::orderBy('id', 'DESC')
            ->offset($start)
            ->limit($limit);
        } else {
            $search = $request->search['value'];
            $query = ProductBrand::where('name', 'like', '%'.$search.'%')
            ->orderBy('id', 'DESC')
            ->offset($start)
            ->limit($limit);

            $total_data = ProductBrand::where('name', 'like', '%'.$search.'%')
        ...
        if(count($query->get()) > 0) {
            ...
            foreach($query->get() as $value) {
                $arr_data = array(
                    ...
                    'action' => '<a href="'.route('master.product_brand.detail', ['id' => $value->id]) .'"
    ...
    {
        return view('master.product_brand.create');
    }
    ...
    {
        $data = ProductBrand::where('id', $id)->first();
        
        if(empty($data)) {
            return $this->errorPage(404,'Data tidak ditemukan');
        }

        return view('master.product_brand.detail', compact('data'));
    }
    ...
    public function update(StoreUpdateRequest $request, $id)
    {
        try {
            ...
            ProductBrand::where('id', $id)->update($input);
